...
$(function(){
	if ($('.limit-text').length) {
		...
		$(".limit-text").keyup(function(){
			var $chars_about_us_multiple = $length_max - $(".limit-text").val().length;
			$(".counter-text").html($chars_about_us_multiple + " caracteres restantes");
			if($chars_about_us_multiple <= 20 && $chars_about_us_multiple > 0)
				$(".counter-text").css("color","#F00");
			else
				$(".counter-text").css("color","#666");
			if($chars_about_us_multiple <= 0){
