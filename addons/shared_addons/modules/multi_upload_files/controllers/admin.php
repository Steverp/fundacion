...
class Admin extends Admin_Controller {
    ...
    public function upload_file($idCategory) {
        ...
        if ($statusJson != "error") {
            ...
            $configArchivo['encrypt_name'] = TRUE;
            ...
            for ($i = 0; $i < $cpt; $i++) {
                ...
                /* Insertamos las rutas en la base de datos */
                ...
                if (!$this->upload->do_upload($file_element_name)) {
                    ...
                    $ruta = 'uploads/default/multi_upload_files/' . $idCategory . '/' . $datosArchivo['raw_name'] . $datosArchivo['file_ext'];
                    $data = array('category' => $idCategory, 'image' => $ruta, 'extencion' => $datosArchivo['file_ext']);
                    $this->db->insert('multi_upload_files', $data);
    ...
    public function renombrarArchivo($id) {
        ...
        $msgJson = "";
        $nuevo_nombre = "";
        /* Armamos un nombre de ruta que coincida con el de base de datos para poder cambiar su nombre */
        $ruta_old_name = 'uploads/default/multi_upload_files/' . $id . '/' . $nombreOrig;
        $ruta_new_name = 'uploads/default/multi_upload_files/' . $id . '/' . $nuevoNombre . '.' . $extensionArchivo;
        ...
        if (!file_exists($archivoAdjunto)) {
            if (!empty($nuevoNombre)) {
                ...
                $sql = 'SELECT * FROM default_multi_upload_files WHERE category = ? AND image = ?';
                $id_img = $this->db->query($sql, array($id, $ruta_old_name))->row();
                if (!empty($id_img)) {
                    $data = array(
                        'image' => $ruta_new_name
                    );
                    $this->db->where('id', $id_img->id)->update('multi_upload_files', $data);
                }
                $statusJson = "success";
    ...
    public function delete_file($idcarpeta) {
        ...
        $ruta = 'uploads/default/multi_upload_files/' . $idcarpeta . '/' . $nomfile;

        if ($this->multi_upload_file_model->delete_file($this->moduleName, $idcarpeta, $nomfile)) {
            $delet_img = $this->db->where('category', $idcarpeta)->where('image', $ruta)->delete('multi_upload_files');
            $statusJson = 'success';
