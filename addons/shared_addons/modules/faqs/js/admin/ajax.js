...
$(document).ready(function()
{
	...
	$(document).on("click ", ".outstanding_faq", function(event)
	{
	...
	$(document).on("click ", ".promotions_faq", function(event)
	{
		event.preventDefault();
		imageSrc = $(this).find('img').attr('src');
		element = $(this);
    ...
    {*/
   	...
   	function Sortable(element, subCatArray, idParent)
   	{
		...
		({
			type: "POST",
			dataType: "json",
			url: base_url+'admin/faqs/orden_categories/',
   	...
   	({
        ...
        stop:   function(event, ui)
        {
            ...
            {
            	sortable_faqs($(this), subCatArrayProd);
            }
        },
   	});
   	
   	function sortable_faqs(element, subCatArray)
   	{
		$.ajax
		({
			type: "POST",
			dataType: "json",
			url: base_url+'admin/faqs/orden_faqs/',
