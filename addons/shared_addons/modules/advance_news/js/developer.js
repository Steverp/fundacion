...
$(function(){
	if ($('.limit-text').length) {
		...
		$(".limit-text").keyup(function(){
			var $chars_advance_new = $length_max - $(".limit-text").val().length;
			$(".counter-text").html($chars_advance_new + " caracteres restantes");
			if($chars_advance_new <= 20 && $chars_advance_new > 0)
				$(".counter-text").css("color","#F00");
			else
				$(".counter-text").css("color","#666");
			if($chars_advance_new <= 0){
