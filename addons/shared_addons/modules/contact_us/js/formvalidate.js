$(document).ready(function()
{
    ...
    $('.form_contac_ajax2').validate({
        rules: {
            id_subject: {required: true},
        ...
        submitHandler: function(event)
        {
            llamadaAjaxContact('.form_contac_ajax2');  // mandamos el nombre del formulario
        }
    });
    ...
    function llamadaAjaxContact(formname)
    {
        $.ajax({
            ...
            beforeSend: function()
            {
                $('#loading_contacts2').html('<img src="' + base_url + 'uploads/default/loading.gif" width="28" height="28"/>');
            },
            success: function(data)
            {
                $('#loading_contacts2').html(data.msg);
                if (data.status == 'error') {
                    $('#loading_contacts2').html('<div style="color: red">' + data.msg + '</div>');
                } else {
                    $('#loading_contacts2').html('<div style="color: green">' + data.msg + '</div>');
