...
function build_categories($rows, $module = null, $parent = 0, $ban = true, $current = null, $slugFather = '', $level = 1, $selCategory = null) {
    ...
    if ($ban)
        $result.= "<li id='todos' class='list-group-item'><a href='{{uri}}'><i class='fa fa-list-ul'></i> Todos</a></li>";
    foreach ($rows as $row) {
        if ($row->parent == $parent) {
            ...
            $result.= "<li id='" . $row->slug . '_' . $row->id . "' class='" . ($children ? $plusClass : '') . ' ' . ($row->parent == 0 ? $classCategories : ($children ? $classSubcategoriesFather : $classSubcategories)) . ' ' . ($row->slug == $selCategory ? $classActive : '') . ($level < 2 ? ' list-group-item' : '') . " '>
				<a href='{{uri}}/index/" . $row->slug . "'>" . $row->title . "</a>";
            if ($children = true) {
...
$menu = build_categories($categories, '{{uri}}', 0, true, $current, '', 1, $selCategory);
?>
...
<section class="cont_home m_b_30">
    <section class="marco">
        ...
        <div class="marco">
            <div class="titulos">
                <h1>{{ if {session:data name="lang_code"} == 'en'}}Files{{ else }}Archivos {{ endif }}</h1>
            </div>
            <div class="clear"></div>
        </div>
        <!--Fin Titulo-->  
                    ...
                    <li class="large-4 medium-4 small-12 columns wow flipInX">
                        ...
                        <div class="marco_prod"> 
                            <a href="{{doc}}" target="_blank">
                                <i class="{{img_doc}}"></i>
                                <div class="vermas_prd">{{ name }}</div>
                            </a>
                            <a href="{{doc}}" download="{{name}}">Descargar<i class="fa fa-arrow-down"></i></a>
                        </div>
                        <hr>
                        {{endif}}
                    </li>
